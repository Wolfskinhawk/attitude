# 1 "./src/main/sensors/gyroanalyse.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/sensors/gyroanalyse.c"
# 18 "./src/main/sensors/gyroanalyse.c"
...
# 19 "./src/main/sensors/gyroanalyse.c" 2
...
# 21 "./src/main/sensors/gyroanalyse.c" 2
...
# 24 "./src/main/sensors/gyroanalyse.c" 2
...
# 26 "./src/main/sensors/gyroanalyse.c" 2
...
# 28 "./src/main/sensors/gyroanalyse.c" 2
...
# 29 "./src/main/sensors/gyroanalyse.c" 2
...
# 30 "./src/main/sensors/gyroanalyse.c" 2
...
#pragma GCC diagnostic pop
# 33 "./src/main/sensors/gyroanalyse.c" 2
...
# 34 "./src/main/sensors/gyroanalyse.c" 2
...
# 36 "./src/main/sensors/gyroanalyse.c" 2
...
# 37 "./src/main/sensors/gyroanalyse.c" 2
# 53 "./src/main/sensors/gyroanalyse.c"
static uint16_t samplingFrequency;
static uint8_t fftBinCount;
static float fftResolution;
static float gyroData[3][32];

static arm_rfft_fast_instance_f32 fftInstance;
static float fftData[32];
static float rfftData[32];
static gyroFftData_t fftResult[3];
static uint16_t fftMaxFreq = 0;
...
static float hanningWindow[32];

void initHanning(void)
{
    for (int i = 0; i < 32; i++) {
        hanningWindow[i] = (0.5 - 0.5 * cos_approx(2 * 3.14159265358979323846f * i / (32 - 1)));
    }
}
...
void gyroDataAnalyseInit(uint32_t targetLooptimeUs)
{
    ...
    fftSamplingScale = samplingFrequency / 1000;
    fftMaxFreq = 1000 / 2;
    fftBinCount = fftFreqToBin(fftMaxFreq) + 1;
    fftResolution = 1000 / 32;
    arm_rfft_fast_init_f32(&fftInstance, 32);
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        biquadFilterInitLPF(&fftFreqFilter[axis], 60, looptime);
        biquadFilterInit(&fftGyroFilter[axis], 200, 1000000 / 1000, 1.0f / sqrtf(2.0f), FILTER_BPF);
    }
}
...
void gyroDataAnalyse(const gyroDev_t *gyroDev, biquadFilter_t *notchFilterDyn)
{
    ...
    if (fftAccCount == fftSamplingScale) {
        ...
        for (int axis = 0; axis < 3; axis++) {
            ...
            if (axis == 0)
                {if (debugMode == (DEBUG_FFT)) {debug[(2)] = (lrintf(sample * gyroDev->scale));}};
            fftAcc[axis] = 0;
        }
...
void gyroDataAnalyseUpdate(biquadFilter_t *notchFilterDyn)
{
    ...
    {if (debugMode == (DEBUG_FFT_TIME)) {debug[(0)] = (step);}};
    switch (step) {
        case STEP_ARM_CFFT_F32:
        {
            ...
            case 64:
                ...
                arm_radix8_butterfly_f32(fftData, 32 / 2, Sint->pTwiddle, 1);
                break;
            }
            {if (debugMode == (DEBUG_FFT_TIME)) {debug[(1)] = (micros() - startTime);}};
            break;
        }
        ...
        case STEP_ARM_CMPLX_MAG_F32:
        {
            ...
            {if (debugMode == (DEBUG_FFT_TIME)) {debug[(2)] = (micros() - startTime);}};
            step++;
            __attribute__ ((fallthrough));
        }
        case STEP_CALC_FREQUENCIES:
        {
            ...
            for (int i = 0; i < fftBinCount; i++) {
                ...
                fftResult[axis].maxVal = __extension__ ({ __typeof__ (fftResult[axis].maxVal) _a = (fftResult[axis].maxVal); __typeof__ (squaredData) _b = (squaredData); _a > _b ? _a : _b; });
                fftSum += squaredData;
            ...
            if (fftSum > 0) {
                ...
                float centerFreq;
                centerFreq = constrain(fftMeanIndex * fftResolution, 120 + 10, fftMaxFreq);
                centerFreq = biquadFilterApply(&fftFreqFilter[axis], centerFreq);
                centerFreq = constrain(centerFreq, 120 + 10, fftMaxFreq);
                fftResult[axis].centerFreq = centerFreq;
                if (axis == 0) {
                    {if (debugMode == (DEBUG_FFT)) {debug[(3)] = (lrintf(fftMeanIndex * 100));}};
                }
            }

            {if (debugMode == (DEBUG_FFT_FREQ)) {debug[(axis)] = (fftResult[axis].centerFreq);}};
            {if (debugMode == (DEBUG_FFT_TIME)) {debug[(1)] = (micros() - startTime);}};
            break;
        }
        case STEP_UPDATE_FILTERS:
        {
            ...
            float cutoffFreq = constrain(fftResult[axis].centerFreq - 100, 120, 200);
            float notchQ = ((float)(cutoffFreq * fftResult[axis].centerFreq) / ((float)(fftResult[axis].centerFreq - cutoffFreq) * (float)(fftResult[axis].centerFreq + cutoffFreq)));
