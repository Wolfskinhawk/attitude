...
# 20 "./src/main/drivers/sdcard.c" 2
...
# 22 "./src/main/drivers/sdcard.c" 2
...
# 26 "./src/main/drivers/sdcard.c" 2
...
# 27 "./src/main/drivers/sdcard.c" 2
...
# 28 "./src/main/drivers/sdcard.c" 2
...
# 30 "./src/main/drivers/sdcard.c" 2
...
# 31 "./src/main/drivers/sdcard.c" 2
...
# 33 "./src/main/drivers/sdcard.c" 2
# 1 "./src/main/drivers/sdcard_standard.h" 1
# 18 "./src/main/drivers/sdcard_standard.h"
...
# 240 "./src/main/drivers/sdcard_standard.h"
uint32_t readBitfield(uint8_t *buffer, unsigned bitIndex, unsigned bitLen);
# 34 "./src/main/drivers/sdcard.c" 2
# 55 "./src/main/drivers/sdcard.c"
typedef enum {
...
# 90 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 90 "./src/main/drivers/sdcard.c"
        highCapacity;
...
# 104 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 104 "./src/main/drivers/sdcard.c"
        enabled;
    
# 105 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 105 "./src/main/drivers/sdcard.c"
        detectionInverted;
    
# 106 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 106 "./src/main/drivers/sdcard.c"
        useDMAForTx;
    IO_t cardDetectPin;
    IO_t chipSelectPin;
...
typedef char assert_failed_sdcard_csd_bitfields_didnt_pack_properly [(sizeof(sdcardCSD_t) == 16) ? 1 : -1 ] __attribute__((unused));

void sdcardInsertionDetectDeinit(void)
{
    if (sdcard.cardDetectPin) {
        IOInit(sdcard.cardDetectPin, OWNER_FREE, 0);
        IOConfigGPIO(sdcard.cardDetectPin, ((GPIO_Mode_IN) | ((0) << 2) | ((0) << 4) | ((GPIO_PuPd_NOPULL) << 5)));
    }
}
...
void sdcardInsertionDetectInit(void)
{
    if (sdcard.cardDetectPin) {
        ...
        IOConfigGPIO(sdcard.cardDetectPin, ((GPIO_Mode_IN) | ((0) << 2) | ((0) << 4) | ((GPIO_PuPd_UP) << 5)));
    }
}
...
# 136 "./src/main/drivers/sdcard.c" 3 4
_Bool 
# 136 "./src/main/drivers/sdcard.c"
    sdcard_isInserted(void)
{
    
# 138 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 138 "./src/main/drivers/sdcard.c"
        result = 
# 138 "./src/main/drivers/sdcard.c" 3 4
                 1
# 138 "./src/main/drivers/sdcard.c"
...
# 152 "./src/main/drivers/sdcard.c" 3 4
_Bool 
# 152 "./src/main/drivers/sdcard.c"
    sdcard_isFunctional(void)
{
...
static void sdcard_reset(void)
{
    ...
    if (sdcard.state >= SDCARD_STATE_READY) {
        spiSetDivisor(sdcard.instance, 128);
    }

    sdcard.failureCount++;
    if (sdcard.failureCount >= 8) {
        sdcard.state = SDCARD_STATE_NOT_PRESENT;
    } else {
...
static 
# 204 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 204 "./src/main/drivers/sdcard.c"
           sdcard_waitForIdle(int maxBytesToWait)
{
    while (maxBytesToWait > 0) {
        uint8_t b = spiTransferByte(sdcard.instance, 0xFF);
        if (b == 0xFF) {
            return 
# 209 "./src/main/drivers/sdcard.c" 3 4
                  1
# 209 "./src/main/drivers/sdcard.c"
                      ;
        }
        maxBytesToWait--;
    }
    ...
    return 
# 214 "./src/main/drivers/sdcard.c" 3 4
          0
# 214 "./src/main/drivers/sdcard.c"
               ;
}
...
static uint8_t sdcard_sendCommand(uint8_t commandCode, uint32_t commandArgument)
{
    ...
    if (!sdcard_waitForIdle(8) && commandCode != 0)
        return 0xFF;

    spiTransfer(sdcard.instance, command, 
# 260 "./src/main/drivers/sdcard.c" 3 4
                                         ((void *)0)
# 260 "./src/main/drivers/sdcard.c"
                                             , sizeof(command));
...
static 
# 280 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 280 "./src/main/drivers/sdcard.c"
           sdcard_validateInterfaceCondition(void)
{
    ...
    uint8_t status = sdcard_sendCommand(8, (0x01 << 8) | 0xAB);
...
# 296 "./src/main/drivers/sdcard.c" 3 4
                                    ((void *)0)
# 296 "./src/main/drivers/sdcard.c"
                                        , ifCondReply, sizeof(ifCondReply));





        if (ifCondReply[3] == 0xAB) {
            sdcard.version = 2;
        }
    }
...
static 
# 312 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 312 "./src/main/drivers/sdcard.c"
           sdcard_readOCRRegister(uint32_t *result)
{
    sdcard_select();

    uint8_t status = sdcard_sendCommand(58, 0);

    uint8_t response[4];

    spiTransfer(sdcard.instance, 
# 320 "./src/main/drivers/sdcard.c" 3 4
                                ((void *)0)
# 320 "./src/main/drivers/sdcard.c"
    ...
    if (status == 0) {
        ...
        *result = (response[0] << 24) | (response[1] << 16) | (response[2] << 8) | response[3];
        ...
        return 
# 327 "./src/main/drivers/sdcard.c" 3 4
              1
# 327 "./src/main/drivers/sdcard.c"
                  ;
    } else {
        sdcard_deselect();

        return 
# 331 "./src/main/drivers/sdcard.c" 3 4
              0
# 331 "./src/main/drivers/sdcard.c"
                   ;
    }
}
...
static sdcardReceiveBlockStatus_e sdcard_receiveDataBlock(uint8_t *buffer, int count)
{
    ...
    spiTransfer(sdcard.instance, 
# 358 "./src/main/drivers/sdcard.c" 3 4
                                ((void *)0)
# 358 "./src/main/drivers/sdcard.c"
                                    , buffer, count);
...
static 
# 367 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 367 "./src/main/drivers/sdcard.c"
           sdcard_sendDataBlockFinish(void)
{
...
static void sdcard_sendDataBlockBegin(const uint8_t *buffer, 
# 393 "./src/main/drivers/sdcard.c" 3 4
                                                            _Bool 
# 393 "./src/main/drivers/sdcard.c"
                                                                 multiBlockWrite)
{

    spiTransferByte(sdcard.instance, 0xFF);

    spiTransferByte(sdcard.instance, multiBlockWrite ? 0xFC : 0xFE);

    if (sdcard.useDMAForTx) {
# 430 "./src/main/drivers/sdcard.c"
        ...
        init.DMA_M2M = ((uint32_t)0x00000000);
        init.DMA_MemoryBaseAddr = (uint32_t) buffer;
        init.DMA_DIR = ((uint32_t)0x00000010);

        init.DMA_PeripheralBaseAddr = (uint32_t) &sdcard.instance->DR;
        init.DMA_Priority = ((uint32_t)0x00000000);
        init.DMA_PeripheralInc = ((uint32_t)0x00000000);
        init.DMA_PeripheralDataSize = ((uint32_t)0x00000000);

        init.DMA_MemoryInc = ((uint32_t)0x00000080);
        init.DMA_MemoryDataSize = ((uint32_t)0x00000000);

        init.DMA_BufferSize = 512;
        init.DMA_Mode = ((uint32_t)0x00000000);
        ...
        SPI_I2S_DMACmd(sdcard.instance, ((uint16_t)0x0002), ENABLE);

    } else {

        spiTransfer(sdcard.instance, buffer, 
# 462 "./src/main/drivers/sdcard.c" 3 4
                                            ((void *)0)
# 462 "./src/main/drivers/sdcard.c"
                                                , 256);
    }
}

static 
# 466 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 466 "./src/main/drivers/sdcard.c"
           sdcard_receiveCID(void)
{
    uint8_t cid[16];

    if (sdcard_receiveDataBlock(cid, sizeof(cid)) != SDCARD_RECEIVE_SUCCESS) {
        return 
# 471 "./src/main/drivers/sdcard.c" 3 4
              0
# 471 "./src/main/drivers/sdcard.c"
                   ;
    }
    ...
    return 
# 487 "./src/main/drivers/sdcard.c" 3 4
          1
# 487 "./src/main/drivers/sdcard.c"
              ;
}

static 
# 490 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 490 "./src/main/drivers/sdcard.c"
           sdcard_fetchCSD(void)
{
...
# 500 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 500 "./src/main/drivers/sdcard.c"
        success =
        sdcard_sendCommand(9, 0) == 0
        && sdcard_receiveDataBlock((uint8_t*) &sdcard.csd, sizeof(sdcard.csd)) == SDCARD_RECEIVE_SUCCESS
        && readBitfield(sdcard.csd.data, 127, 1) == 1;

    if (success) {
        switch (readBitfield(sdcard.csd.data, 0, 2)) {
            case 0:

                readBlockLen = 1 << readBitfield(sdcard.csd.data, 44, 4);
                blockCountMult = 1 << (readBitfield(sdcard.csd.data, 78, 3) + 2);
                blockCount = (readBitfield(sdcard.csd.data, 54, 12) + 1) * blockCountMult;


                capacityBytes = (uint64_t) blockCount * readBlockLen;


                sdcard.metadata.numBlocks = capacityBytes / 512;
            break;
            case 1:
                sdcard.metadata.numBlocks = (readBitfield(sdcard.csd.data, 58, 22) + 1) * 1024;
            break;
            default:
                success = 
# 523 "./src/main/drivers/sdcard.c" 3 4
                         0
# 523 "./src/main/drivers/sdcard.c"
                              ;
        }
    }
...
static 
# 537 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 537 "./src/main/drivers/sdcard.c"
           sdcard_checkInitDone(void)
{
    sdcard_select();

    uint8_t status = sdcard_sendAppCommand(41, sdcard.version == 2 ? 1 << 30 : 0);
...
void sdcard_init(const sdcardConfig_t *config)
{
    ...
    if (config->chipSelectTag) {
        ...
        IOConfigGPIO(sdcard.chipSelectPin, ((GPIO_Mode_OUT) | ((GPIO_Speed_Level_3) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)));
    } else {
        sdcard.chipSelectPin = ((IO_t)0);
    }
    ...
    if (config->cardDetectTag) {
        ...
        sdcard.cardDetectPin = ((IO_t)0);
        sdcard.detectionInverted = 
# 584 "./src/main/drivers/sdcard.c" 3 4
                                  0
# 584 "./src/main/drivers/sdcard.c"
                                       ;
    }
    ...
    spiTransfer(sdcard.instance, 
# 596 "./src/main/drivers/sdcard.c" 3 4
                                ((void *)0)
# 596 "./src/main/drivers/sdcard.c"
                                    , 
# 596 "./src/main/drivers/sdcard.c" 3 4
                                      ((void *)0)
# 596 "./src/main/drivers/sdcard.c"
...
static 
# 613 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 613 "./src/main/drivers/sdcard.c"
           sdcard_setBlockLength(uint32_t blockLen)
{
    sdcard_select();

    uint8_t status = sdcard_sendCommand(16, blockLen);
...
static 
# 627 "./src/main/drivers/sdcard.c" 3 4
      _Bool 
# 627 "./src/main/drivers/sdcard.c"
           sdcard_isReady(void)
{
    return sdcard.state == SDCARD_STATE_READY || sdcard.state == SDCARD_STATE_WRITING_MULTIPLE_BLOCKS;
}
# 642 "./src/main/drivers/sdcard.c"
static sdcardOperationStatus_e sdcard_endWriteBlocks(void)
{
    ...
    spiTransferByte(sdcard.instance, 0xFD);
...
# 668 "./src/main/drivers/sdcard.c" 3 4
_Bool 
# 668 "./src/main/drivers/sdcard.c"
    sdcard_poll(void)
{
    if (!sdcard.enabled) {
        sdcard.state = SDCARD_STATE_NOT_PRESENT;
        return 
# 672 "./src/main/drivers/sdcard.c" 3 4
              0
# 672 "./src/main/drivers/sdcard.c"
                   ;
    }

    uint8_t initStatus;
    
# 676 "./src/main/drivers/sdcard.c" 3 4
   _Bool 
# 676 "./src/main/drivers/sdcard.c"
        sendComplete;
    ...
    switch (sdcard.state) {
        case SDCARD_STATE_RESET:
            ...
            initStatus = sdcard_sendCommand(0, 0);
        ...
        case SDCARD_STATE_CARD_INIT_IN_PROGRESS:
            if (sdcard_checkInitDone()) {
                if (sdcard.version == 2) {
                    ...
                    sdcard.highCapacity = 
# 718 "./src/main/drivers/sdcard.c" 3 4
                                         0
# 718 "./src/main/drivers/sdcard.c"
                                              ;
                }
                ...
                if (sdcard_fetchCSD()) {
                    ...
                    uint8_t status = sdcard_sendCommand(10, 0);
        ...
        case SDCARD_STATE_INITIALIZATION_RECEIVE_CID:
            if (sdcard_receiveCID()) {
                ...
                if (!sdcard.highCapacity && !sdcard_setBlockLength(512)) {
                    sdcard_reset();
                    goto doMore;
                }


                spiSetDivisor(sdcard.instance, 2);
        ...
        case SDCARD_STATE_SENDING_WRITE:
            ...
            sendComplete = 
# 763 "./src/main/drivers/sdcard.c" 3 4
                          0
# 763 "./src/main/drivers/sdcard.c"
                               ;
# 786 "./src/main/drivers/sdcard.c"
            if (sdcard.useDMAForTx && DMA_GetFlagStatus(sdcard.dma->completeFlag) == SET) {
                ...
                while (SPI_I2S_GetFlagStatus(sdcard.instance, ((uint16_t)0x0001)) == SET) {
                    sdcard.instance->DR;
                }
                ...
                SPI_I2S_DMACmd(sdcard.instance, ((uint16_t)0x0002), DISABLE);

                sendComplete = 
# 803 "./src/main/drivers/sdcard.c" 3 4
                              1
# 803 "./src/main/drivers/sdcard.c"
                                  ;
            }

            if (!sdcard.useDMAForTx) {

                spiTransfer(sdcard.instance, sdcard.pendingOperation.buffer + 256 * sdcard.pendingOperation.chunkIndex, 
# 808 "./src/main/drivers/sdcard.c" 3 4
                                                                                                                                             ((void *)0)
# 808 "./src/main/drivers/sdcard.c"
                                                                                                                                                 , 256);

                sdcard.pendingOperation.chunkIndex++;

                sendComplete = sdcard.pendingOperation.chunkIndex == 512 / 256;
            }
            ...
            if (sendComplete) {
                ...
                if (sdcard_sendDataBlockFinish()) {
                    ...
                    if (sdcard.pendingOperation.callback) {
                        sdcard.pendingOperation.callback(SDCARD_BLOCK_OPERATION_WRITE, sdcard.pendingOperation.blockIndex, 
# 834 "./src/main/drivers/sdcard.c" 3 4
                                                                                                                          ((void *)0)
# 834 "./src/main/drivers/sdcard.c"
        ...
        case SDCARD_STATE_WAITING_FOR_WRITE:
            ...
            } else if (millis() > sdcard.operationStartTime + 250) {
        ...
        case SDCARD_STATE_READING:
            switch (sdcard_receiveDataBlock(sdcard.pendingOperation.buffer, 512)) {
                ...
                case SDCARD_RECEIVE_BLOCK_IN_PROGRESS:
                    if (millis() <= sdcard.operationStartTime + 100) {
                        break;
                    }
...
# 923 "./src/main/drivers/sdcard.c" 3 4
                           ((void *)0)
# 923 "./src/main/drivers/sdcard.c"
                               ,
                            sdcard.pendingOperation.callbackData
                        );
                    }
    ...
    if (sdcard.state >= SDCARD_STATE_RESET && sdcard.state < SDCARD_STATE_READY
            && millis() - sdcard.operationStartTime > 200) {
        sdcard_reset();
    }

    return sdcard_isReady();
}
# 976 "./src/main/drivers/sdcard.c"
sdcardOperationStatus_e sdcard_writeBlock(uint32_t blockIndex, uint8_t *buffer, sdcard_operationCompleteCallback_c callback, uint32_t callbackData)
{
    uint8_t status;
    ...
    switch (sdcard.state) {
        ...
        case SDCARD_STATE_READY:
            ...
            status = sdcard_sendCommand(24, sdcard.highCapacity ? blockIndex : blockIndex * 512);
...
# 1043 "./src/main/drivers/sdcard.c"
sdcardOperationStatus_e sdcard_beginWriteBlocks(uint32_t blockIndex, uint32_t blockCount)
{
    if (sdcard.state != SDCARD_STATE_READY) {
    ...
    if (
        sdcard_sendAppCommand(23, blockCount) == 0
        && sdcard_sendCommand(25, sdcard.highCapacity ? blockIndex : blockIndex * 512) == 0
    ) {
        sdcard.state = SDCARD_STATE_WRITING_MULTIPLE_BLOCKS;
...
# 1091 "./src/main/drivers/sdcard.c"

# 1091 "./src/main/drivers/sdcard.c" 3 4
_Bool 
# 1091 "./src/main/drivers/sdcard.c"
    sdcard_readBlock(uint32_t blockIndex, uint8_t *buffer, sdcard_operationCompleteCallback_c callback, uint32_t callbackData)
{
    if (sdcard.state != SDCARD_STATE_READY) {
        if (sdcard.state == SDCARD_STATE_WRITING_MULTIPLE_BLOCKS) {
            if (sdcard_endWriteBlocks() != SDCARD_OPERATION_SUCCESS) {
                return 
# 1096 "./src/main/drivers/sdcard.c" 3 4
                      0
# 1096 "./src/main/drivers/sdcard.c"
                           ;
            }
        } else {
            return 
# 1099 "./src/main/drivers/sdcard.c" 3 4
                  0
# 1099 "./src/main/drivers/sdcard.c"
                       ;
        }
    }
    ...
    uint8_t status = sdcard_sendCommand(17, sdcard.highCapacity ? blockIndex : blockIndex * 512);
    ...
    if (status == 0) {
        ...
        return 
# 1124 "./src/main/drivers/sdcard.c" 3 4
              1
# 1124 "./src/main/drivers/sdcard.c"
                  ;
    } else {
        sdcard_deselect();

        return 
# 1128 "./src/main/drivers/sdcard.c" 3 4
              0
# 1128 "./src/main/drivers/sdcard.c"
                   ;
    }
}
...
# 1135 "./src/main/drivers/sdcard.c" 3 4
_Bool 
# 1135 "./src/main/drivers/sdcard.c"
    sdcard_isInitialized(void)
{
