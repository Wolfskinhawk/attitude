# 1 "./src/main/drivers/serial_uart_init.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/drivers/serial_uart_init.c"
# 29 "./src/main/drivers/serial_uart_init.c"
# 1 "d:\\gcc-arm-none-eabi-7-2017-q4-major-win32\\lib\\gcc\\arm-none-eabi\\7.2.1\\include\\stdbool.h" 1 3 4
# 30 "./src/main/drivers/serial_uart_init.c" 2
...
# 31 "./src/main/drivers/serial_uart_init.c" 2
...
# 33 "./src/main/drivers/serial_uart_init.c" 2

# 1 "./src/main/build/build_config.h" 1
# 18 "./src/main/build/build_config.h"
       
# 35 "./src/main/drivers/serial_uart_init.c" 2
...
# 28 "./src/main/drivers/inverter.h"
                                               on);
# 38 "./src/main/drivers/serial_uart_init.c" 2
...
# 39 "./src/main/drivers/serial_uart_init.c" 2
...
# 42 "./src/main/drivers/serial_uart_init.c" 2
...
# 43 "./src/main/drivers/serial_uart_init.c" 2
...
# 48 "./src/main/drivers/serial_uart_init.c" 3 4
   _Bool 
# 48 "./src/main/drivers/serial_uart_init.c"
        inverted = uartPort->port.options & SERIAL_INVERTED;
# 58 "./src/main/drivers/serial_uart_init.c"
    uint32_t inversionPins = 0;

    if (uartPort->port.mode & MODE_TX) {
        inversionPins |= ((uint32_t)0x00020000);
    }
    if (uartPort->port.mode & MODE_RX) {
        inversionPins |= ((uint32_t)0x00010000);
    }
...
void uartReconfigure(uartPort_t *uartPort)
{
    ...
    if (
        ...
        USART_InitStructure.USART_WordLength = ((uint32_t)0x00001000);
    } else {
        USART_InitStructure.USART_WordLength = ((uint32_t)0x00000000);
    }

    USART_InitStructure.USART_StopBits = (uartPort->port.options & SERIAL_STOPBITS_2) ? ((uint32_t)0x00002000) : ((uint32_t)0x00000000);
    USART_InitStructure.USART_Parity = (uartPort->port.options & SERIAL_PARITY_EVEN) ? ((uint32_t)0x00000400) : ((uint32_t)0x00000000);

    USART_InitStructure.USART_HardwareFlowControl = ((uint32_t)0x00000000);
    USART_InitStructure.USART_Mode = 0;
    if (uartPort->port.mode & MODE_RX)
        USART_InitStructure.USART_Mode |= ((uint32_t)0x00000004);
    if (uartPort->port.mode & MODE_TX)
        USART_InitStructure.USART_Mode |= ((uint32_t)0x00000008);
...
serialPort_t *uartOpen(UARTDevice_e device, serialReceiveCallbackPtr rxCallback, void *rxCallbackData, uint32_t baudRate, portMode_e mode, portOptions_e options)
{
    ...
    s->txDMAEmpty = 
# 121 "./src/main/drivers/serial_uart_init.c" 3 4
                   1
# 121 "./src/main/drivers/serial_uart_init.c"
                       ;
    ...
    if (mode & MODE_RX) {
# 152 "./src/main/drivers/serial_uart_init.c"
        if (s->rxDMAChannel) {
...
# 175 "./src/main/drivers/serial_uart_init.c"
            ...
            USART_DMACmd(s->USARTx, ((uint32_t)0x00000040), ENABLE);
            s->rxDMAPos = DMA_GetCurrDataCounter(s->rxDMAChannel);

        } else {
            USART_ClearITPendingBit(s->USARTx, ((uint32_t)0x00050105));
            USART_ITConfig(s->USARTx, ((uint32_t)0x00050105), ENABLE);
        }
    }


    if (mode & MODE_TX) {
# 206 "./src/main/drivers/serial_uart_init.c"
        if (s->txDMAChannel) {
            DMA_StructInit(&DMA_InitStructure);
...
# 227 "./src/main/drivers/serial_uart_init.c"
            ...
            DMA_ITConfig(s->txDMAChannel, ((uint32_t)0x00000002), ENABLE);
            DMA_SetCurrDataCounter(s->txDMAChannel, 0);
            s->txDMAChannel->CNDTR = 0;

            USART_DMACmd(s->USARTx, ((uint32_t)0x00000080), ENABLE);
        } else {
            USART_ITConfig(s->USARTx, ((uint32_t)0x00070107), ENABLE);
        }
    }
