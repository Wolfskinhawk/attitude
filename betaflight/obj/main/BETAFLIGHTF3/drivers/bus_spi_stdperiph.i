# 1 "./src/main/drivers/bus_spi_stdperiph.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/drivers/bus_spi_stdperiph.c"
# 18 "./src/main/drivers/bus_spi_stdperiph.c"
# 1 "d:\\gcc-arm-none-eabi-7-2017-q4-major-win32\\lib\\gcc\\arm-none-eabi\\7.2.1\\include\\stdbool.h" 1 3 4
# 19 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 20 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 21 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 23 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 27 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 28 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 29 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 41 "./src/main/drivers/exti.h"
                             enable);
# 30 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 31 "./src/main/drivers/bus_spi_stdperiph.c" 2
...
# 32 "./src/main/drivers/bus_spi_stdperiph.c" 2

spiDevice_t spiDevice[3];
...
void spiInitDevice(SPIDevice device)
{
    ...
    if (spi->dev == ((SPI_TypeDef *) (((uint32_t)0x40000000) + 0x00003800))) {
        spi->leadingEdge = 
# 41 "./src/main/drivers/bus_spi_stdperiph.c" 3 4
                          1
# 41 "./src/main/drivers/bus_spi_stdperiph.c"
                              ;
    }
# 51 "./src/main/drivers/bus_spi_stdperiph.c"
    RCC_ClockCmd(spi->rcc, ENABLE);
    RCC_ResetCmd(spi->rcc, ENABLE);

    IOInit(IOGetByTag(spi->sck), OWNER_SPI_SCK, (device + 1));
    IOInit(IOGetByTag(spi->miso), OWNER_SPI_MISO, (device + 1));
    IOInit(IOGetByTag(spi->mosi), OWNER_SPI_MOSI, (device + 1));


    IOConfigGPIOAF(IOGetByTag(spi->sck), ((GPIO_Mode_AF) | ((GPIO_Speed_Level_3) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)), spi->af);
    IOConfigGPIOAF(IOGetByTag(spi->miso), ((GPIO_Mode_AF) | ((GPIO_Speed_Level_3) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)), spi->af);
    IOConfigGPIOAF(IOGetByTag(spi->mosi), ((GPIO_Mode_AF) | ((GPIO_Speed_Level_3) << 2) | ((GPIO_OType_PP) << 4) | ((GPIO_PuPd_NOPULL) << 5)), spi->af);
# 71 "./src/main/drivers/bus_spi_stdperiph.c"
    SPI_I2S_DeInit(spi->dev);

    SPI_InitTypeDef spiInit;
    spiInit.SPI_Mode = ((uint16_t)0x0104);
    spiInit.SPI_Direction = ((uint16_t)0x0000);
    spiInit.SPI_DataSize = ((uint16_t)0x0700);
    spiInit.SPI_NSS = ((uint16_t)0x0200);
    spiInit.SPI_FirstBit = ((uint16_t)0x0000);
    spiInit.SPI_CRCPolynomial = 7;
    spiInit.SPI_BaudRatePrescaler = ((uint16_t)0x0010);

    if (spi->leadingEdge) {
        spiInit.SPI_CPOL = ((uint16_t)0x0000);
        spiInit.SPI_CPHA = ((uint16_t)0x0000);
    } else {
        spiInit.SPI_CPOL = ((uint16_t)0x0002);
        spiInit.SPI_CPHA = ((uint16_t)0x0001);
    }



    SPI_RxFIFOThresholdConfig(spi->dev, ((uint16_t)0x1000));
...
uint8_t spiTransferByte(SPI_TypeDef *instance, uint8_t txByte)
{
    ...
    while (SPI_I2S_GetFlagStatus(instance, ((uint16_t)0x0002)) == RESET)
        if ((spiTimeout--) == 0)
    ...
    spiTimeout = 1000;
    while (SPI_I2S_GetFlagStatus(instance, ((uint16_t)0x0001)) == RESET)
        if ((spiTimeout--) == 0)
            return spiTimeoutUserCallback(instance);
...
# 128 "./src/main/drivers/bus_spi_stdperiph.c" 3 4
_Bool 
# 128 "./src/main/drivers/bus_spi_stdperiph.c"
    spiIsBusBusy(SPI_TypeDef *instance)
{

    return SPI_GetTransmissionFIFOStatus(instance) != ((uint16_t)0x0000) || SPI_I2S_GetFlagStatus(instance, ((uint16_t)0x0080)) == SET;
...
# 138 "./src/main/drivers/bus_spi_stdperiph.c" 3 4
_Bool 
# 138 "./src/main/drivers/bus_spi_stdperiph.c"
    spiTransfer(SPI_TypeDef *instance, const uint8_t *txData, uint8_t *rxData, int len)
{
    uint16_t spiTimeout = 1000;
    ...
    return 
# 169 "./src/main/drivers/bus_spi_stdperiph.c" 3 4
          1
# 169 "./src/main/drivers/bus_spi_stdperiph.c"
              ;
}

void spiSetDivisor(SPI_TypeDef *instance, uint16_t divisor)
{
# 184 "./src/main/drivers/bus_spi_stdperiph.c"
    SPI_Cmd(instance, DISABLE);

    const uint16_t tempRegister = (instance->CR1 & ~(((1 << (5)) | (1 << (4)) | (1 << (3)))));
