...
# 41 "./src/main/rx/sbus.c" 2
# 72 "./src/main/rx/sbus.c"
enum {
...
typedef union sbusFrame_u {
    uint8_t bytes[(sizeof(sbusChannels_t) + 2)];
...
# 102 "./src/main/rx/sbus.c" 3 4
   _Bool 
# 102 "./src/main/rx/sbus.c"
        done;
} sbusFrameData_t;
...
static void sbusDataReceive(uint16_t c, void *data)
{
    ...
    if (sbusFrameTime > (long)(3000 + 500)) {
    ...
    if (sbusFrameData->position < (sizeof(sbusChannels_t) + 2)) {
        sbusFrameData->frame.bytes[sbusFrameData->position++] = (uint8_t)c;
        if (sbusFrameData->position < (sizeof(sbusChannels_t) + 2)) {
            sbusFrameData->done = 
# 129 "./src/main/rx/sbus.c" 3 4
                                 0
# 129 "./src/main/rx/sbus.c"
                                      ;
        } else {
            sbusFrameData->done = 
# 131 "./src/main/rx/sbus.c" 3 4
                                 1
# 131 "./src/main/rx/sbus.c"
                                     ;
            {if (debugMode == (DEBUG_SBUS)) {debug[(DEBUG_SBUS_FRAME_TIME)] = (sbusFrameTime);}};
        }
    }
}
...
static uint8_t sbusFrameStatus(rxRuntimeConfig_t *rxRuntimeConfig)
{
    ...
    sbusFrameData->done = 
# 143 "./src/main/rx/sbus.c" 3 4
                         0
# 143 "./src/main/rx/sbus.c"
                              ;

    {if (debugMode == (DEBUG_SBUS)) {debug[(DEBUG_SBUS_FRAME_FLAGS)] = (sbusFrameData->frame.frame.channels.flags);}};

    if (sbusFrameData->frame.frame.channels.flags & (1 << 2)) {
        sbusFrameData->stateFlags |= (1 << 1);
        {if (debugMode == (DEBUG_SBUS)) {debug[(DEBUG_SBUS_STATE_FLAGS)] = (sbusFrameData->stateFlags);}};
    }
    if (sbusFrameData->frame.frame.channels.flags & (1 << 3)) {
        sbusFrameData->stateFlags |= (1 << 0);
        {if (debugMode == (DEBUG_SBUS)) {debug[(DEBUG_SBUS_STATE_FLAGS)] = (sbusFrameData->stateFlags);}};
    }
...
# 161 "./src/main/rx/sbus.c" 3 4
_Bool 
# 161 "./src/main/rx/sbus.c"
    sbusInit(const rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig)
{
    ...
    if (!portConfig) {
        return 
# 177 "./src/main/rx/sbus.c" 3 4
              0
# 177 "./src/main/rx/sbus.c"
                   ;
    }


    
# 181 "./src/main/rx/sbus.c" 3 4
   _Bool 
# 181 "./src/main/rx/sbus.c"
    ...
    serialPort_t *sBusPort = openSerialPort(portConfig->identifier,
        ...
        (SERIAL_STOPBITS_2 | SERIAL_PARITY_EVEN) | (rxConfig->serialrx_inverted ? 0 : SERIAL_INVERTED) | (rxConfig->halfDuplex ? SERIAL_BIDIR : 0)
        );
    ...
    return sBusPort != 
# 201 "./src/main/rx/sbus.c" 3 4
                      ((void *)0)
# 201 "./src/main/rx/sbus.c"
                          ;
}
