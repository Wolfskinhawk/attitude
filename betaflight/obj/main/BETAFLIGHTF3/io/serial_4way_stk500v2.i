# 1 "./src/main/io/serial_4way_stk500v2.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/io/serial_4way_stk500v2.c"
# 21 "./src/main/io/serial_4way_stk500v2.c"
# 1 "d:\\gcc-arm-none-eabi-7-2017-q4-major-win32\\lib\\gcc\\arm-none-eabi\\7.2.1\\include\\stdbool.h" 1 3 4
# 22 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 23 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 25 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 29 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 30 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 31 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 33 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 34 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 36 "./src/main/io/serial_4way_stk500v2.c" 2
...
# 53 "./src/main/io/serial_4way_stk500v2.c"
...
# 86 "./src/main/io/serial_4way_stk500v2.c"
...
static int8_t ReadBit(void)
{
    ...
    uint32_t timeout_timer = btimer + 250;
    while (isEscHi(selected_esc)) {if (micros() > timeout_timer) goto timeout;};
    while (isEscLo(selected_esc)) {if (micros() > timeout_timer) goto timeout;};
    LastBitTime = micros() - btimer;
    if (LastBitTime <= HiLoTsh) {
        timeout_timer = timeout_timer + 250;
        while (isEscHi(selected_esc)) {if (micros() > timeout_timer) goto timeout;};
        while (isEscLo(selected_esc)) {if (micros() > timeout_timer) goto timeout;};
...
static uint8_t StkReadLeader(void)
{
    ...
    if ((StkCmd == 0x15) || (StkCmd == 0x12)) {
         waitcycl = ((1000 / 250) * 5000);
    } else if (StkCmd == 0x01) {
        waitcycl = ((1000 / 250) / 2);
    } else {
        waitcycl= ((1000 / 250) * 35);
    }
...
static uint8_t StkRcvPacket(uint8_t *pstring)
{
    ...
    ckSumIn=0;
    if (!ReadByte(&bt) || (bt != 0x1B)) goto Err;
    ...
    if (!ReadByte(&bt) || (bt != 0x0E)) goto Err;
    if (!ReadByte(&bt) || (bt != StkCmd)) goto Err;
    if (!ReadByte(&bt) || (bt != 0x00)) goto Err;
...
static uint8_t _CMD_SPI_MULTI_EX(volatile uint8_t * ResByte,uint8_t Cmd,uint8_t AdrHi,uint8_t AdrLo)
{
    ...
    StkSendByte(0);
    StkSendPacketFooter();
    if (StkRcvPacket((void *)StkInBuf)) {
         if ((StkInBuf[0] == 0x00) && ((StkInBuf[1] == Cmd)||(StkInBuf[1] == 0x00) ) &&(StkInBuf[2] == 0x00)) {
            *ResByte = StkInBuf[3];
         }
         return 1;
    }
...
{
    if (Stk_SignOn()) {
        if (_CMD_SPI_MULTI_EX(&pDeviceInfo->bytes[1], 0x30,0,1)) {
            if (_CMD_SPI_MULTI_EX(&pDeviceInfo->bytes[0], 0x30,0,2)) {
                return 1;
            }
        }
    }
