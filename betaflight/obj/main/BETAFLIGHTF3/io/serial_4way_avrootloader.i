# 1 "./src/main/io/serial_4way_avrootloader.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "./src/main/io/serial_4way_avrootloader.c"
# 21 "./src/main/io/serial_4way_avrootloader.c"
# 1 "d:\\gcc-arm-none-eabi-7-2017-q4-major-win32\\lib\\gcc\\arm-none-eabi\\7.2.1\\include\\stdbool.h" 1 3 4
# 22 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 23 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 24 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 26 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 30 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 31 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 32 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 33 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 35 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 36 "./src/main/io/serial_4way_avrootloader.c" 2
...
# 38 "./src/main/io/serial_4way_avrootloader.c" 2
# 71 "./src/main/io/serial_4way_avrootloader.c"
static uint8_t suart_getc_(uint8_t *bt)
{
    uint32_t btime;
    uint32_t start_time;

    uint32_t wait_time = millis() + 2;
...
uint8_t BL_ConnectEx(uint8_32_u *pDeviceInfo)
{
    ...
    pDeviceInfo->bytes[2] = BootInfo[4 - 1];
    pDeviceInfo->bytes[1] = BootInfo[(4)];
    pDeviceInfo->bytes[0] = BootInfo[(4 +1)];
    return (1);
}
...
static uint8_t BL_SendCMDSetAddress(ioMem_t *pMem)
{
    ...
    uint8_t sCMD[] = {0xFF, 0, pMem->D_FLASH_ADDR_H, pMem->D_FLASH_ADDR_L };
    BL_SendBuf(sCMD, 4);
    return (BL_GetACK(2) == 0x30);
}

static uint8_t BL_SendCMDSetBuffer(ioMem_t *pMem)
{
    uint8_t sCMD[] = {0xFE, 0, 0, pMem->D_NUM_BYTES};
    if (pMem->D_NUM_BYTES == 0) {

        sCMD[2] = 1;
    }
    BL_SendBuf(sCMD, 4);
    if (BL_GetACK(2) != 0xFF) return 0;
    BL_SendBuf(pMem->D_PTR_I, pMem->D_NUM_BYTES);
    return (BL_GetACK(40) == 0x30);
}
...
{
    if (interface_mode == 2) {
        return BL_ReadA(0x07, pMem);
    } else {
        return BL_ReadA(0x03, pMem);
    }
}

uint8_t BL_ReadEEprom(ioMem_t *pMem)
{
    return BL_ReadA(0x04, pMem);
}

uint8_t BL_PageErase(ioMem_t *pMem)
{
    if (BL_SendCMDSetAddress(pMem)) {
        uint8_t sCMD[] = {0x02, 0x01};
        BL_SendBuf(sCMD, 2);
        return (BL_GetACK((1400 / 2)) == 0x30);
    }
    return 0;
}
...
uint8_t BL_WriteEEprom(ioMem_t *pMem)
{
    return BL_WriteA(0x05, pMem, (3000 / 2));
}

uint8_t BL_WriteFlash(ioMem_t *pMem)
{
    return BL_WriteA(0x01, pMem, (500 / 2));
}
...
uint8_t BL_VerifyFlash(ioMem_t *pMem)
{
    if (BL_SendCMDSetAddress(pMem)) {
        if (!BL_SendCMDSetBuffer(pMem)) return 0;
        uint8_t sCMD[] = {0x04, 0x01};
        BL_SendBuf(sCMD, 2);
        return (BL_GetACK(40 / 2));
    }
    return 0;
}
